...
const Scroll = (props) => {
  ...
  menuItems.forEach(
    (section) => (sectionsRefs[section?.props?.id] = createRef())
  );
  ...
  const CurrentScrolledSection = ({ sectionsWrapperRef, children }) => {
    ...
    useEffect(() => {
      if (sectionsWrapperRef.current) {
        const tElements = [...sectionsWrapperRef.current.children].reduce(
          (map, item) => {
    ...
    }, [throttledOnScroll, sectionsWrapperRef, targetElements]);
